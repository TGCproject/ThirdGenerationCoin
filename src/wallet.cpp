...
// ThirdGenerationCoin: optional setting to unlock wallet for block minting only;
...
// ThirdGenerationCoin: total coins staked (non-spendable until maturity)
    ...
    BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
    {
      ...
      for(unsigned int i = 0; i < pcoin->vout.size(); i++)
      {
        ...
        if(pcoin->nTime > nSpendTime)
          continue;  // ThirdGenerationCoin: timestamp must not exceed spend time
...

  {
    ...
    CTxDB txdb("r");
    {
      ...
      for(;;)
      {
        ...
        // ThirdGenerationCoin: sub-cent change is moved to fee
        ...
        if(nChange > 0)
        {
          ...
          if(!GetBoolArg("-avatar")) // ThirdGenerationCoin: not avatar mode
          {
...
// ThirdGenerationCoin: create coin stake transaction
...
// ThirdGenerationCoin: check 'spent' consistency between wallet and txindex
// ThirdGenerationCoin: fix wallet spent state according to txindex
...
// ThirdGenerationCoin: disable transaction (only for coinstake)
