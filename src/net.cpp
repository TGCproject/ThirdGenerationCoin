...
{
  // Wait for IRC to get it first - disabled with ThirdGenerationCoin
...
void ThreadMapPort2(void* parg)
{
  ...
  if(r == 1)
  {
    ...
    string strDesc = "ThirdGenerationCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
    /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are ThirdGenerationCoin dns seeds.
static const char *strDNSSeed[][2] = {
  {"115.28.141.131", "115.28.141.131"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
  ...
  if(true /*!fTestNet*/)  // ThirdGenerationCoin enables dns seeding with testnet too
  {
...
// ThirdGenerationCoin: stake minter thread
void static ThreadStakeMinter(void *parg)
{
  ...
  try
  {
    ...
    ThirdGenerationCoinMiner(pwallet, true);
    vnThreadsRunning[THREAD_MINTER]--;
  }
  catch (std::exception& e) {
    vnThreadsRunning[THREAD_MINTER]--;
...
// ThirdGenerationCoin: burn miner after coins were burnt
void static ThreadAfterBurner(void *parg)
{
  ...
  try
  {
    ...
    ThirdGenerationCoinAfterBurner(pwallet);
    vnThreadsRunning[THREAD_BURNER]--;
  }
  catch (std::exception& e) {
    vnThreadsRunning[THREAD_BURNER]--;
...
bool BindListenPort(string& strError)
{
  ...
  if(::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
  {
    ...
    if(nErr == WSAEADDRINUSE)
      strError = strprintf(_("Unable to bind to port %d on this computer.  ThirdGenerationCoin is probably already running."), ntohs(sockaddr.sin_port));
    else
...
void StartNode(void* parg)
{
  ...
  // IRC disabled with ThirdGenerationCoin
  ...
  GenerateThirdGenerationCoins(GetBoolArg("-gen", false), pwalletMain);

  // ThirdGenerationCoin: mint proof-of-stake blocks in the background
  if(!CreateThread(ThreadStakeMinter, pwalletMain))
    printf("Error: CreateThread(ThreadStakeMinter) failed\n");

  // ThirdGenerationCoin: hash proof-of-burn in the background
...
bool StopNode()
{
  ...
  if(vnThreadsRunning[THREAD_MINER] > 0)             printf("ThreadThirdGenerationCoinMiner still running\n");
