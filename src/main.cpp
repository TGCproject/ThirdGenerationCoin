...
const string strMessageMagic = "ThirdGenerationCoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
  if(!fConnect)
  {
    // ThirdGenerationCoin: wallets need to refund inputs when disconnecting coinstake
    if(tx.IsCoinStake())
    {
      BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
        if(pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
  ...
  for(int i = 0; i < vout.size(); i++)
  {
    ...
    // ThirdGenerationCoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
  ...
  // ThirdGenerationCoin: coinstake is also only valid in a block, not as a loose transaction
  if(tx.IsCoinStake())
...
// ThirdGenerationCoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(u32int nBits, bool fProofOfBurn)
{
  ...
  // ThirdGenerationCoin: subsidy is cut in half every 16x multiply of difficulty
...
// ThirdGenerationCoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// ThirdGenerationCoin: find last block index up to pindex
...
static u32int GetNextTargetRequired(const CBlockIndex *pindexLast, bool fProofOfStake)
{
  ...
  // ThirdGenerationCoin: target change every block
  // ThirdGenerationCoin: retarget with exponential moving toward target spacing
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
  ...
  // ThirdGenerationCoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
  ...
  if(!IsCoinBase())
  {
    ...
    for(unsigned int i = 0; i < vin.size(); i++)
    {
      ...
      // ThirdGenerationCoin: check transaction timestamp
    ...
    if(IsCoinStake())
    {
      // ThirdGenerationCoin: coin stake tx earns reward instead of paying fee
      uint64 nCoinAge;
    ...
    else
    {
      ...
      // ThirdGenerationCoin: enforce transaction fees for every block
      if(nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
  ...
  // ThirdGenerationCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
  ...
  // ThirdGenerationCoin: track money supply and mint amount info
  ...
  // ThirdGenerationCoin: fees are not collected by miners as in bitcoin
  // ThirdGenerationCoin: fees are destroyed to compensate the entire network
  if(fDebug && GetBoolArg("-printcreation"))
...
// ThirdGenerationCoin: total coin age spent in transaction, in the unit of coin-days.
...
// ThirdGenerationCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
  ...
  // ThirdGenerationCoin: compute chain trust score
  pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

  // ThirdGenerationCoin: compute stake entropy bit for stake modifier
  if(!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
    return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

  // ThirdGenerationCoin: record proof-of-stake hash value
  ...
  // ThirdGenerationCoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
  ...
  // ThirdGenerationCoin: only the second transaction can be the optional coinstake
  for(int i = 2; i < vtx.size(); i++)
    if(vtx[i].IsCoinStake())
      return DoS(100, error("CheckBlock() : coinstake in wrong position"));

  // ThirdGenerationCoin: coinbase output should be empty if proof-of-stake block
  ...
  // ThirdGenerationCoin: check block signature
...
bool CBlock::AcceptBlock()
{
  ...
  // ThirdGenerationCoin: check that the block satisfies synchronized checkpoint
  ...
  // ThirdGenerationCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode *pfrom, CBlock *pblock)
{
  ...
  // ThirdGenerationCoin: check proof-of-stake
  ...
  // ThirdGenerationCoin: check proof-of-burn
  ...
  // ThirdGenerationCoin: verify hash target and signature of coinstake tx
  ...
  // ThirdGenerationCoin: ask for pending sync-checkpoint if any
  ...
  if(!mapBlockIndex.count(pblock->hashPrevBlock))
  {
    ...
    if(pfrom)
    {
      ...
      // ThirdGenerationCoin: getblocks may not obtain the ancestor block rejected
  ...
  // ThirdGenerationCoin: if responsible for sync-checkpoint send it
...
// ThirdGenerationCoin: sign block
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
  ...
  if(nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
  {
    ...
    printf("*** %s\n", strMessage.c_str());
    ThreadSafeMessageBox(strMessage, "ThirdGenerationCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
    StartShutdown();
    return false;
  }
  return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
  ...
  if(mapBlockIndex.empty())
  {
    ...
    // Genesis block
    const char *pszTimestamp = "There is a byte reveral thing in the miner for the hash.";
    CTransaction txNew;
    txNew.nTime = !fTestNet ? 1402149175 : 1402149175;
    txNew.vin.resize(1);
    txNew.vout.resize(1);
    ...
    block.nVersion = 1;
    block.nTime    = !fTestNet ? 1402149175 : 1402149175;
    block.nBits    = bnProofOfWorkLimit.GetCompact();
    block.nNonce   = !fTestNet ? 3013444 : 3013444;
    ...
    if(fTestNet)
      ...
      assert(block.hashMerkleRoot == uint256("0x83adb72af52afd875a4705990ae36b6d80a6f19234346f7403efd0d82b2a03ba"));
...
//////////////////////////////////////////////////////////////////////////////// 
    ...
    // ThirdGenerationCoin: initialize synchronized checkpoint
    if(!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
      return error("LoadBlockIndex() : failed to init sync checkpoint");
  }

  // ThirdGenerationCoin: if checkpoint master key changed must reset sync-checkpoint
  {
    CTxDB txdb;
    string strPubKey = "";
...
string GetWarnings(string strFor)
{
  ...
  // ThirdGenerationCoin: wallet lock warning for minting
  if(strMintWarning != "")
  {
    nPriority = 0;
  ...
  // ThirdGenerationCoin: should not enter safe mode for longer invalid chain
  // ThirdGenerationCoin: if sync-checkpoint is too old do not enter safe mode
  ...
  // ThirdGenerationCoin: if detected invalid checkpoint enter safe mode
  ...
  // Alerts
  {
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
    {
      ...
      if(alert.AppliesToMe() && alert.nPriority > nPriority)
      {
        ...
        if(nPriority > 1000)
          strRPC = strStatusBar;  // ThirdGenerationCoin: safe mode for high alert
      }
    }
  }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
  ...
  if(strCommand == "version")
  {
    ...
    // ThirdGenerationCoin: record my external IP reported by peer
    ...
    // ThirdGenerationCoin: relay sync-checkpoint
    {
      ...
      if(inv.type == MSG_BLOCK)
      {
        ...
        if(mi != mapBlockIndex.end())
        {
          ...
          if(inv.hash == pfrom->hashContinue)
          {
            ...
            // ThirdGenerationCoin: send latest proof-of-work block to allow the
  ...
  else if(strCommand == "getblocks")
  {
    ...
    for(; pindex; pindex = pindex->pnext)
    {
      if(pindex->GetBlockHash() == hashStop)
      {
        ...
        // ThirdGenerationCoin: tell downloading node about the latest block if it's
...
{
  /*ThirdGenerationCoin: a burn hash is calculated by:
...
CBlock *CreateNewBlock(CWallet* pwallet, bool fProofOfStake, const CWalletTx *burnWalletTx, CReserveKey *resKey)
{
  ...
  // ThirdGenerationCoin: if coinstake available add coinstake tx
  ...
  int64 nFees = 0;
  {
    ...
    while(!mapPriority.empty())
    {
      ...
      // ThirdGenerationCoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock *pblock, CWallet &wallet, CReserveKey &reservekey)
{
  ...
  if(pblock->IsProofOfWork() && hash > hashTarget)
  {
    ...
    return error("ThirdGenerationCoinMiner : proof-of-work not meeting target");
  }

  if(pblock->IsProofOfBurn() && burnHash > hashBurnTarget)
    return error("ThirdGenerationCoinMiner : proof-of-burn not meeting target");
  ...
  // Found a solution
  {
    ...
    if(pblock->hashPrevBlock != hashBestChain)
      return error("ThirdGenerationCoinMiner : generated block is stale");
    ...
    if(!ProcessBlock(NULL, pblock))
      return error("ThirdGenerationCoinMiner : ProcessBlock, block not accepted");
  }

  return true;
}

void static ThreadThirdGenerationCoinMiner(void* parg);

static bool fGenerateThirdGenerationCoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void ThirdGenerationCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
  ...
  while(fGenerateThirdGenerationCoins || fProofOfStake)
  {
    if(fShutdown)
      return;
    ...
    while(vNodes.empty() || IsInitialBlockDownload())
    {
      ...
      if(fShutdown)
        return;
      if((!fGenerateThirdGenerationCoins) && !fProofOfStake)
        return;
    }
    ...
    if(fProofOfStake)
    {
      // ThirdGenerationCoin: if proof-of-stake block found then process block
      if(pblock->IsProofOfStake())
      {
    ...
    printf("Running ThirdGenerationCoinMiner with %d %s in block\n", 
    ...
    for(;;)
    {
      ...
      if(fShutdown)
        return;
      if(!fGenerateThirdGenerationCoins)
        return;
...
void ThirdGenerationCoinAfterBurner(CWallet *pwallet)
{
  ...
  for(;;)
  {
    ...
    if(pindexLastBlock != pindexBest)
    {
      ...
      //debug print
      printf("ThirdGenerationCoinAfterBurner():\n");
...
void static ThreadThirdGenerationCoinMiner(void* parg)
{
  ...
  try
  {
    ...
    ThirdGenerationCoinMiner(pwallet, false);
    vnThreadsRunning[THREAD_MINER]--;
  }catch(std::exception& e)
  {
    vnThreadsRunning[THREAD_MINER]--;
    PrintException(&e, "ThreadThirdGenerationCoinMiner()");
  }catch(...)
  {
    vnThreadsRunning[THREAD_MINER]--;
    PrintException(NULL, "ThreadThirdGenerationCoinMiner()");
  }
  ...
  printf("ThreadThirdGenerationCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}

void GenerateThirdGenerationCoins(bool fGenerate, CWallet* pwallet)
{
  fGenerateThirdGenerationCoins = fGenerate;
  nLimitProcessors = GetArg("-genproclimit", -1);

  if(!nLimitProcessors)
    fGenerateThirdGenerationCoins = false;
  ...
  if(fGenerate)
  {
    ...
    printf("Starting %d ThirdGenerationCoinMiner threads\n", nAddThreads);

    for(int i = 0; i < nAddThreads; i++)
    {
      if(!CreateThread(ThreadThirdGenerationCoinMiner, pwallet))
        printf("Error: CreateThread(ThreadThirdGenerationCoinMiner) failed\n");
