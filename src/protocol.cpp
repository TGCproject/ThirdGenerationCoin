...
// ThirdGenerationCoin message start (switch from Bitcoin's in v0.2)
static unsigned char pchMessageStartBitcoin[4] = { 0x9f, 0xeb, 0x1b, 0x8a };
static unsigned char pchMessageStartThirdGenerationCoin[4] = { 0x6e, 0x8b, 0x92, 0xa5 };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
  if(fTestNet)
    ...
    memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartThirdGenerationCoin : pchMessageStartBitcoin, sizeof(pchMessageStartThirdGenerationCoin));
