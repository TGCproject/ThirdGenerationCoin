...
Value stop(const Array& params, bool fHelp)
{
  if(fHelp || params.size() != 0)
    throw runtime_error(
      "stop\n"
      "Stop ThirdGenerationCoin server.");
  // Shutdown will take long enough that the response should get back
  StartShutdown();
  return "ThirdGenerationCoin server stopping";
}
...
Value setgenerate(const Array& params, bool fHelp)
{
  ...
  GenerateThirdGenerationCoins(fGenerate, pwalletMain);
  return Value::null;
}
...
// ThirdGenerationCoin: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
  if(fHelp || params.size() > 1)
    throw runtime_error(
      ...
      "Returns a new ThirdGenerationCoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
  if(fHelp || params.size() != 1)
    throw runtime_error(
      ...
      "Returns the current ThirdGenerationCoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
  if(fHelp || params.size() < 1 || params.size() > 2)
    throw runtime_error(
      "setaccount <ThirdGenerationCoinaddress> <account>\n"
      "Sets the account associated with the given address.");

  CBitcoinAddress address(params[0].get_str());
  if(!address.IsValid())
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ThirdGenerationCoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
  if(fHelp || params.size() != 1)
    throw runtime_error(
      "getaccount <ThirdGenerationCoinaddress>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
  if(pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
    throw runtime_error(
      "sendtoaddress <ThirdGenerationCoinaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
  if(fHelp || params.size() != 2)
    throw runtime_error(
      "signmessage <ThirdGenerationCoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
  if(fHelp || params.size() != 3)
    throw runtime_error(
      "verifymessage <ThirdGenerationCoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
  if(fHelp || params.size() < 1 || params.size() > 2)
    throw runtime_error(
      "getreceivedbyaddress <ThirdGenerationCoinaddress> [minconf=1]\n"
      "Returns the total amount received by <ThirdGenerationCoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array &params, bool fHelp)
{
  if(pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
    throw runtime_error(
      "sendfrom <fromaccount> <toThirdGenerationCoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
      "<amount> is a real and is rounded to the nearest 0.000001\n"
      "requires wallet passphrase to be set with walletpassphrase first");
  if(!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
    throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
  ...
  BOOST_FOREACH(const Pair& s, sendTo)
  {
    ...
    if(!address.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid ThirdGenerationCoin address:")+s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
  ...
  // ThirdGenerationCoin: if user OS account compromised prevent trivial sendmoney commands
  if(params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
  ...
  StartShutdown();
  return "wallet encrypted; ThirdGenerationCoin server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
  if(fHelp || params.size() != 1)
    throw runtime_error(
      "validateaddress <ThirdGenerationCoinaddress>\n"
      "Return information about <ThirdGenerationCoinaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
  ...
  if(vNodes.empty())
    throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ThirdGenerationCoin is not connected!");

  if(IsInitialBlockDownload())
    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ThirdGenerationCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
  ...
  {
    if(vNodes.empty())
      throw JSONRPCError(-9, "ThirdGenerationCoin is not connected!");

    if(IsInitialBlockDownload())
      throw JSONRPCError(-10, "ThirdGenerationCoin is downloading blocks...");
...
// ThirdGenerationCoin: get information of sync-checkpoint
...
// ThirdGenerationCoin: reserve balance from being staked for network protection
...
// ThirdGenerationCoin: check wallet integrity
...
// ThirdGenerationCoin: repair wallet
...
// ThirdGenerationCoin: make a public-private key pair
...
// ThirdGenerationCoin: send alert.  
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
  ostringstream s;
  s << "POST / HTTP/1.1\r\n"
    << "User-Agent: ThirdGenerationCoin-json-rpc/" << FormatFullVersion() << "\r\n"
    << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg)
{
  if(nStatus == 401)
    return strprintf("HTTP/1.0 401 Authorization Required\r\n"
                     "Date: %s\r\n"
                     "Server: ThirdGenerationCoin-json-rpc/%s\r\n"
...
void ThreadRPCServer2(void* parg)
{
  ...
  if(mapArgs["-rpcpassword"] == "")
  {
    ...
    string strWhatAmI = "To use ThirdGenerationCoind";
